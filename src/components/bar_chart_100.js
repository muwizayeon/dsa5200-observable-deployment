...
export function drawBars100(dataset, metric, plotwidth, labelDicts, tooltipId) {
  ...
  function onMouseEnter(e, datum) {
    ...
    tooltip.style(
      "transform",
      `translate(` +
        `calc( 350% + ${x}px),` + // CSS calc() calculates an offset based on values with different units 498%, 2220%
        `calc( -70% + ${y}px)` + // e.g. Offset our tooltip up half of its own width (-50%) and left -100%
        `)`,
